...
export default function Home() {
  ...
  const checkout = async () => {
    ...
    try {
      ...
      // 3. Проверяем наличие для каждого товара
      const validItems: CartItem[] = [];
      const productsToUpdate: Product[] = [];
      
      for (const [productIdStr, variants] of Object.entries(toDeduct)) {
        const productId = Number(productIdStr);
        const product = freshProducts.find(p => p.id === productId);
        ...
        // Создаём глубокую копию продукта
        const productCopy: Product = JSON.parse(JSON.stringify(product));
        let hasChanges = false;
        let allOk = true;
        
        for (const [variantName, qtyNeeded] of Object.entries(variants)) {
          const variant = productCopy.variants.find((v: Variant) => v.name === variantName);
        ...
        if (hasChanges) {
          productsToUpdate.push(productCopy);
        }
      }
      ...
      // 6. АТОМАРНО обновляем каждый товар в базе
      for (const updatedProduct of productsToUpdate) {
        const totalStock = updatedProduct.variants.reduce((s: number, v: Variant) => s + v.stock, 0);
        ...
        if (updateError) {
          console.error('UPDATE error for product', updatedProduct.id, updateError);
          showNotification('Ошибка обновления: ' + updateError.message, 'error');
        } else {
          console.log('✅ Updated product', updatedProduct.id, 'result:', updateResult);
        }
      }
      
      // 7. Обновляем локальный стейт
      const finalProducts = freshProducts.map(p => {
        const updated = productsToUpdate.find(u => u.id === p.id);
        return updated || p;
      });
      setProducts(finalProducts);
              ...
              <div className="space-y-2">
                ...
                {products.filter(p => {
                  ...
                  <div className="glass-panel p-8 text-center text-gray-500">
                    ...
                    <p className="text-xs">Товаров нет</p>
                  </div>
                )}
              </div>
            </>
