...
const INITIAL_PRODUCTS = [
  { 
    ...
    flavors: [
      { name: 'Манго', stock: 15 },
      { name: 'Виноград', stock: 8 },
      { name: 'Холодок', stock: 0 },
      { name: 'Яблоко', stock: 12 },
      { name: 'Персик', stock: 5 },
      { name: 'Клубника', stock: 0 },
      { name: 'Арбуз', stock: 7 },
    ]
  },
  { 
    id: 2, 
    name: 'Жидкость Mango Ice 100ml', 
    category: 'Жидкости', 
    price: 8,
    image: null,
    flavors: [
  ...
  { 
    id: 3, 
    name: 'XROS 3 Pod System', 
    category: 'POD-системы', 
    price: 25,
    image: null,
    flavors: [
      { name: 'Чёрный', stock: 10 },
      { name: 'Серебристый', stock: 8 },
      { name: 'Розовый', stock: 5 },
    ]
  },
  { 
    id: 4, 
    name: 'Снюс Nordic Spirit', 
    category: 'Снюс', 
    price: 3.5,
    image: null,
    flavors: [
      ...
      { name: 'Манго', stock: 9 },
      { name: 'Арбуз', stock: 14 },
      { name: 'Виноград', stock: 7 },
      { name: 'Персик', stock: 11 },
    ]
  },
  { 
    id: 5, 
    name: 'Кальян Alpha Hookah', 
    category: 'Кальяны', 
    price: 45,
    image: null,
    flavors: [
      { name: 'Чёрный матовый', stock: 3 },
      { name: 'Белый', stock: 2 },
    ]
  },
  { 
    id: 6, 
    name: 'Испаритель 0.4ohm', 
    category: 'Расходники', 
    price: 2.5,
    image: null,
    flavors: [
      { name: 'Стандарт', stock: 50 },
    ]
  },
  { 
    id: 7, 
    ...
    flavors: [
      { name: 'Манго', stock: 20 },
      { name: 'Виноград', stock: 15 },
      { name: 'Холодок', stock: 10 },
    ]
  },
  { 
    id: 8, 
    ...
    flavors: [
      { name: 'Чёрный', stock: 8 },
      { name: 'Синий', stock: 6 },
      { name: 'Красный', stock: 0 },
      { name: 'Зелёный', stock: 4 },
      { name: 'Белый', stock: 10 },
      { name: 'Розовый', stock: 7 },
    ]
  },
];
...
export default function Home() {
  ...
  // Проверка возраста при загрузке
  useEffect(() => {
  ...
  const getAvailableStock = (productId: number, flavor: string) => {
  ...
  const openProductModal = (product: Product) => {
    ...
    const firstAvailable = product.flavors.find((f) => {
      const available = getAvailableStock(product.id, f.name);
      return available > 0;
    });
  ...
  // Оформление заказа с глобальным счетчиком
    ...
    try {
      for (const item of cart) {
        ...
        if (!flavorData || flavorData.stock < item.quantity) {
          showNotification(`"${item.productName} (${item.flavor})" - недостаточно в наличии`, 'error');
      ...
      if (countError) {
        console.error('Ошибка получения номера заказа:', countError);
      ...
      if (insertError) {
        console.error('Ошибка сохранения заказа:', insertError);
      ...
      const updatedProducts = products.map(product => {
        ...
        const updatedFlavors = product.flavors.map(flavor => {
          ...
          if (cartItem) {
            return { ...flavor, stock: Math.max(0, flavor.stock - cartItem.quantity) };
          }
          return flavor;
        });
  ...
  const availableStock = selectedProduct && selectedFlavor 
    ? getAvailableStock(selectedProduct.id, selectedFlavor) 
    : 0;
  ...
  // Если возраст не подтвержден - показываем проверку
  if (ageVerified === null) {
    return (
  ...
  // Если пользователь отказался от проверки - показываем экран отказа
  if (ageDeclined) {
    return (
        ...
        <div className="w-full max-w-sm glass-panel p-8 text-center animate-fade-in">
          ...
          <div className="glass-card p-4 mb-6">
            <p className="text-xs text-gray-500">
              Если вы считаете что произошла ошибка, очистите данные браузера и попробуйте снова.
            </p>
          </div>
          
          <button
            onClick={() => {
              localStorage.removeItem('liqvape_age_verified');
              setAgeVerified(null);
    ...
    <div className="min-h-screen text-white relative">
      ...
      {notification && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 pointer-events-none">
          <div 
            className={`w-full max-w-[280px] rounded-2xl p-4 backdrop-blur-2xl border shadow-2xl transition-all duration-300 ${
              notificationVisible 
          ...
          >
            <div className="flex flex-col items-center text-center">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-3 ${
                notification.type === 'error' ? 'bg-red-500/30' : 'bg-green-500/30'
              }`}>
                {notification.type === 'error' ? (
              ...
              <p className={`text-sm font-medium leading-relaxed ${
                notification.type === 'error' ? 'text-red-100' : 'text-green-100'
              }`}>
              ...
              <div className="w-full h-0.5 rounded-full bg-white/10 mt-3 overflow-hidden">
                <div 
                  ...
                  style={{ width: notificationVisible ? '0%' : '100%' }}
                ></div>
              </div>
            </div>
          </div>
        ...
        <div className="pt-4">
          ...
          <div className="relative mb-6 animate-slide-in">
            ...
            <input
              ...
              className="w-full glass-panel py-4 pl-12 pr-4 text-white placeholder-gray-500 transition-all"
              value={search}
            ...
            <div className="grid grid-cols-2 gap-4 pb-8">
              {filteredProducts.map((product, index) => {
                ...
                return (
                  <div
                    key={product.id}
                    onClick={() => available > 0 && openProductModal(product)}
                  ...
                  >
                    ...
                    <h3 className="font-semibold text-sm mb-2 line-clamp-2 text-white group-hover:text-orange-400 transition-colors">
                      {product.name}
                    </h3>
          ...
          {/* Декоративные элементы */}
          <div className="fixed top-20 right-10 w-32 h-32 bg-orange-500/5 rounded-full blur-3xl pointer-events-none"></div>
          <div className="fixed bottom-20 left-10 w-40 h-40 bg-pink-500/5 rounded-full blur-3xl pointer-events-none"></div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/90 backdrop-blur-xl animate-fade-in">
          <div className="w-full max-w-sm glass-panel p-8 text-center">
            ...
            <p className="text-gray-400 text-sm mb-8 leading-relaxed">
              Данный сайт содержит информацию о никотиносодержащей продукции. 
            ...
            <p className="text-xs text-gray-600 mt-6">
              Нажимая "Мне есть 18 лет", вы подтверждаете своё совершеннолетие
            </p>
          </div>
        </div>
      )}
                ...
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <p className="text-xs text-gray-500">
                      ...
                      {selectedProduct.flavors.filter((f) => getAvailableStock(selectedProduct.id, f.name) === 0).length > 0 && (
                        <span className="text-red-500"> • {selectedProduct.flavors.filter((f) => getAvailableStock(selectedProduct.id, f.name) === 0).length} нет</span>
                      )}
                    </p>
                  </div>
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <input
                              ...
                              onChange={() => available > 0 && setSelectedFlavor(flavor.name)}
                            ...
                            <div>
                              ...
                              {inCart > 0 && (
                                <span className="ml-2 text-xs text-orange-400">({inCart} в корзине)</span>
                              )}
                            </div>
                          </div>
                  ...
                  {hiddenFlavorsCount > 0 && !showAllFlavors && (
                    <button
                      ...
                      className="w-full mt-3 py-3 rounded-xl border border-orange-500/30 text-orange-400 text-sm hover:bg-orange-500/10 transition-colors"
                    >
                  ...
                  {showAllFlavors && hiddenFlavorsCount > 0 && (
                    <button
                      ...
                      className="w-full mt-3 py-3 rounded-xl border border-white/10 text-gray-400 text-sm hover:bg-white/5 transition-colors"
                    >
                      ↑ Свернуть
                    </button>
                  )}
              ...
              {isFlavorAvailable ? (
                <>
                  <div className="mb-6">
                        ...
                        <button
                          onClick={() => {
                            if (quantity + 1 > availableStock) {
                              showNotification(`Максимум: ${availableStock} шт.`, 'error');
                            } else {
                    ...
                    {quantity >= availableStock && (
                      <p className="text-xs text-orange-400 mt-2 text-center">
                        Выбрано максимальное количество ({availableStock} шт.)
                      </p>
                    )}
                  </div>
                  ...
                  >
                    ...
                    В корзину • {selectedProduct.price * quantity} BYN
                  </button>
                </>
              ) : (
                <div className="w-full py-4 rounded-2xl font-bold bg-white/5 text-center text-red-400">
            ...
            <div className="p-4">
              {cart.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600 text-sm mt-2">Добавьте товары из каталога</p>
                </div>
              ) : (
                <>
                        ...
                        <div key={index} className="glass-card p-4">
                          <div className="flex items-start justify-between mb-3">
                            <div className="flex-1">
                              ...
                              {isMaxReached && (
                                <p className="text-xs text-orange-400 mt-1">Максимальное количество</p>
                              )}
                            </div>
                            <button
            ...
            <div className="p-4">
              {userOrders.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600 text-sm mt-2">Оформите первый заказ</p>
                </div>
              ) : (
