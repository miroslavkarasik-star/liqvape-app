    ...
    try {
      ...
      try {
        const optimizedCache = parsed.map(p => ({
          ...
          image: p.image, variants: p.variants, is_preorder: p.is_preorder, is_hidden: p.is_hidden
        }));
  ...
  const saveProduct = async () => {
    ...
    try {
      const data = {
        ...
        image: editingProduct.image || null, // Сохраняем только имя файла
        ...
        <div className="max-w-2xl mx-auto relative z-10">
          ...
          {adminTab === 'products' ? (
            <div>
              ...
              <div className="mb-3 relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" /><input type="text" placeholder="Поиск товаров..." value={adminSearch} onChange={e => setAdminSearch(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-sm text-white outline-none" /></div>
              ...
              <div className="space-y-2">
                ...
                {filteredAdminProducts.length === 0 && <div className="bg-white/5 border border-white/10 rounded-xl p-8 text-center text-gray-500"><Package className="w-12 h-12 mx-auto mb-3 opacity-50" /><p>Товаров нет</p></div>}
              </div>
            </div>
          ) : (
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-black/90 backdrop-blur-xl">
              <div className="bg-black border border-white/10 rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto p-5">
                ...
                <div className="mb-4">
                  ...
                  <button 
                    onClick={() => { loadAvailableImages(); setShowImageGallery(true); }}
              ...
              <div className="bg-black border border-white/10 rounded-2xl w-full max-w-2xl max-h-[80vh] overflow-y-auto p-5">
                ...
                {availableImages.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-500 text-xs mt-2">Закиньте PNG/JPG файлы в папку: public/images/products/</p>
                  </div>
                ) : (
