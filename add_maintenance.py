...
if ', []);' in content and content.count('export default function Home() {') == 1:
    # Находим точную строку начала компонента
    target = "export default function Home() {\n"
    
    maintenance_block = """  const maintenanceUntil = new Date('2026-09-20T23:59:59');
...
"""
    ...
    if target in content:
        content = content.replace(target, target + maintenance_block)
        with open('app/page.tsx', 'w', encoding='utf-8') as f:
            f.write(content)
        print("✅ Заглушка успешно и безопасно добавлена!")
    else:
        print("❌ Не удалось найти точку вставки. Проверь файл.")
else:
    print("⚠️ Файл уже содержит изменения или поврежден. Убедись, что сделал git reset --hard HEAD~1")
